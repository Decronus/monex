...
const ProfileSuggest = ({
  ...
  useEffect(() => {
    if (minutes + ADVERTS_LIFETIME - Math.floor(Date.now() / 1000 / 60) <= 0) {
      setAdvertActive(true);
    } else {
      setAdvertActive(false);
    }
  }, [advertActive, minutes]);
    ...
    notDeleted && (
      <S.Wrap>
        ...
        <S.ActionAndStatus>
          <S.Action id={id}>
            {advertActive ? (
              <S.StartIcon onClick={updateAdvertMinutes} id={id}>
                <svg
                  id={id}
                  width="16"
                  height="21"
